...
Run `"$HOME/.cockpit/scripts/ticket-read" <ticket-url> Name Status PRs` with the URL from the marker, and on a non-zero exit `notion-fetch` on it instead. Either way, take:
