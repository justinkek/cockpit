...
Run `"$HOME/.cockpit/scripts/cockpit-cache-query" user-id`. A non-zero exit means the cache is missing or has no such key - fall back to calling `notion-fetch` with `id: "self"`.
...
```sh
"$HOME/.cockpit/scripts/ticket-register" "<epic-url>"
```
...
```sh
"$HOME/.cockpit/scripts/ticket-register-type" epic "<session-name>"
```
