...
2. Only then write `Agent: Session Id` to `claiming-<4 random chars> @ <iso formatted datetime>`, and `Assignee` to the connected user (`"$HOME/.cockpit/scripts/cockpit-cache-query" user-id`).
...
- `command`: `"$HOME/.cockpit/scripts/ticket-watch-board"`
