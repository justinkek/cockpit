...
Read each with `"$HOME/.claude-shared/cockpit-board-id" get <key>`, following `~/.claude-shared/templates/board-ids.md` - it covers the key list, the unrecorded case, and every exit code.
...
Run `"$HOME/.claude-shared/cockpit-cache-query" user-id`. A non-zero exit means the cache is missing or has no such key - fall back to calling `notion-fetch` with `id: "self"`. Store the user ID for use in stub creation (step 1, cases 2 and 3) to auto-assign the ticket.
...
5. A populated relation is read for the epic's status, not only its presence. Run `"$HOME/.claude-shared/cockpit-cache-query" epic-statuses` and find the linked epic's url in what it returns. No page is fetched: the cache already carries every epic's status, and `epics` is the same list with the closed ones dropped.
...
```sh
"$HOME/.claude-shared/ticket-register" "<cockpit-ticket-url>"
```
...
```sh
"$HOME/.claude-shared/ticket-register-type" ticket "<session-name>" "<ticket-type>"
```
...
```sh
"$HOME/.claude-shared/ticket-register-source-ticket" "<source-ticket-url>"
```
...
```sh
"$HOME/.claude-shared/ticket-register-column" "<current-status>"
```
...
- `command`: `"$HOME/.claude-shared/ticket-watch-column" "<ticket-page-id>"`, where `<ticket-page-id>` is the id from the ticket URL. When the session has a source ticket (case 2, registered at step 4.6), pass its page id as a second argument — a person moves whichever card is in front of them, and watching only the cockpit stub misses every move made on the source board.
