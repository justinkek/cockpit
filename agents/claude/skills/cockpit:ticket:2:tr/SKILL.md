...
1. Read reference tickets by running `"$HOME/.claude-shared/cockpit-cache-query" reference-tickets`. On a non-zero exit or an empty list, fetch from the `reference-tickets` id (see `~/.claude-shared/templates/board-ids.md`).
...
1. **Read the ticket** - read the BR section with `"$HOME/.claude-shared/ticket-read" <ticket-url> --section "<heading>"`, and confirm it is present. Exit 7 is the block named under Prerequisite.
