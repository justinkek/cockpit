...
**When not to** — only four columns carry a counter (`In Dev`, `In CR`, `In FR`, `Ready for Validation`), the same gate as `agents/claude/hooks/remind-back-from-column.sh`. If the session's ticket sits anywhere else, or no ticket is registered, skip the recording silently and carry on with the walk.
