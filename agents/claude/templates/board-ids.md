...
| 3    | no board claims this checkout           | `/cockpit:ticket:0:copilot` asks which does |
...
It sits outside the repository because `cockpit-board-id set` and `cockpit-board-claim` write it as the agent runs, and it holds one machine's Notion ids and checkout paths. `cockpit-board-id set` creates it on first use; until then every read exits 5.
