...
2. Read the Sprints view - its id comes from `"$HOME/.claude-shared/cockpit-board-id" get sprints-view`, see `~/.claude-shared/templates/board-ids.md` - with `notion-query-database-view`, and take the row whose `Start Date` <= today <= `End Date`. The database holds a handful of rows, so no filter is needed. Never `notion-fetch` on the data source instead: it returns the schema only and never the rows, so it cannot answer this.
