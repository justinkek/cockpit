...
COLS
  ...
  echo "[auto-advance-cr] A push just landed. Advance the cockpit ticket to ${post_dev_target} (use the cockpit:ticket:x:status skill to walk each column in order), then run \"\$HOME/.claude-shared/ticket-status-confirm cr\" to mark it done. If the ticket is already at or past ${post_dev_target}, just run the confirm command."
  exit 0
fi
