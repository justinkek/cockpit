...
run_hook "$SESSION" '"$HOME/.claude-shared/ticket-register-type" ticket "test session name"'
...
run_hook "$SESSION" '"$HOME/.claude-shared/ticket-register-column" "In Dev"'
...
run_hook "$SESSION" '"$HOME/.claude-shared/ticket-register-source-ticket" "https://app.notion.com/p/source123"'
...
run_hook "$SESSION" '"$HOME/.claude-shared/ticket-register-column" "In Dev" && "$HOME/.claude-shared/ticket-status-confirm" dev'
...
run_hook "$SESSION" '"$HOME/.claude-shared/ticket-register-type" ticket "chained session name" "bug" && echo done'
...
run_hook "$SESSION" '"$HOME/.claude-shared/ticket-register-source-ticket" "https://app.notion.com/p/chained123"; echo done'
...
run_hook "$SESSION" '"$HOME/.claude-shared/ticket-register-column"'
...
# argv sibling covers.)
run_hook "$SESSION" '"$HOME/.claude-shared/ticket-register-column" InDevUnquoted'
...
decision="$(jq -nc --arg sid "$SESSION" --arg cmd '"$HOME/.claude-shared/ticket-register-column" "In CR" && rm -rf /' \
  '{tool_name:"Bash",session_id:$sid,tool_input:{command:$cmd}}' \
