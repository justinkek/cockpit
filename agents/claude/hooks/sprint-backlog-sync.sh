...
1. Fetch the Sprints data source - its id comes from `"$HOME/.claude-shared/cockpit-board-id" get sprints-data-source`, see ~/.claude-shared/templates/board-ids.md - to find the sprint whose Start Date <= today <= End Date. If zero or multiple sprints match, ask the user to specify.
